...
for clean in daily_transactions_split:
  item_clean = []
  for item in clean:
    item_clean.append(item.replace("\n", "").strip(" "))
  transactions_clean.append(item_clean)
...
for colors_i in colors:
  print(
    "Thread Shed sold {0} threads of {1} thread today.".format(color_count(colors_i), colors_i))
